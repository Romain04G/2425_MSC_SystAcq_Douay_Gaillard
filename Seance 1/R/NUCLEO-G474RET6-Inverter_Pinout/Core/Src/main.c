	...
	while (1)
	{
		...
		if(newCmdReady){
		    if (strcmp(argv[0], "help") == 0) {
		        ...
		        //__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, period*0.6);  // Déphasage complet d'une demi-période
		        // Canal 2 : PWM de 50 % de rapport cyclique, déphasé de 180° (une demi-période)
		        //__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, (period-pulse_value)*0.6);  // Déphasage complet d'une demi-période
